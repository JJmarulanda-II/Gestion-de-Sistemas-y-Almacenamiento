...
export default function ControlCaja() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Se carga el historial si es ADMIN
  ...
  // --- FUNCIÓN SENIOR PARA EXPORTAR AUDITORÍA A PDF ---
  const exportarPDF = () => {
    ...
    // Título y Fecha
    doc.setFontSize(18);
    doc.text("Auditoría de Cajas - Pollo Fresh ERP", 14, 20);
    doc.setFontSize(11);
    ...
    const columnas = ["Fecha", "Cajero", "Apertura", "Ventas Sist.", "Cierre Físico", "Diferencia"];
    const filas = historial.map(reg => {
      const dif = parseFloat(reg.diferencia);
      return [
        new Date(reg.fecha_apertura).toLocaleDateString('es-CO'),
        reg.Usuario?.nombre,
        formatoMoneda(reg.monto_apertura),
        reg.total_ventas_calculado ? formatoMoneda(reg.total_ventas_calculado) : '---',
        reg.monto_cierre ? formatoMoneda(reg.monto_cierre) : 'Operando...',
        reg.estado === 'ABIERTA' ? 'En Curso' : (dif === 0 ? 'OK' : formatoMoneda(dif))
      ];
    });
    ...
    autoTable(doc, {
      ...
      startY: 35,
      theme: 'grid',
      headStyles: { fillColor: [16, 185, 129] }, // Verde Esmeralda (Combina con la UI de Caja)
    });

    doc.save(`Auditoria_Cajas_${new Date().getTime()}.pdf`);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {usuario?.rol === 'ADMIN' && (
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg border border-gray-100 overflow-x-auto">
            ...
            {/* --- CABECERA CORREGIDA CON EL BOTÓN --- */}
            <div className="flex justify-between items-center mb-6">
              ...
              <button 
                onClick={exportarPDF}
                className="bg-emerald-600 hover:bg-emerald-800 text-white px-4 py-2 rounded-lg text-sm font-bold transition shadow flex items-center gap-2"
              >
                <span>📄</span> Exportar Cuentas Claras
              </button>
            </div>
